...
class WeatherListViewController: UIViewController {
    ...
    @objc private func selectedCity(notification: NSNotification) {
        ...
        if !myCities.contains(Coordinate(lon: cityCoordinate.latitude.makeRound(),
                                         lat: cityCoordinate.longitude.makeRound())){
            getWeatherByCoordinate(latitude: cityCoordinate.latitude,
                                   longitude: cityCoordinate.longitude)
            myCities.append(Coordinate(lon: cityCoordinate.latitude.makeRound(),
                                       lat: cityCoordinate.longitude.makeRound())
            )
        }
    }
    ...
    private func checkCurrentLocationOrNot(bodyData: WeatherInfo) {
        ...
        if coordinate.latitude.makeRound() == bodyData.coordinate?.lat,
           coordinate.longitude.makeRound() == bodyData.coordinate?.lon {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            if !allowPermission {
                ...
                let coordinate = weather[indexPath.row].coordinate
